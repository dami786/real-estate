      ...
      <div className="mt-6 bg-white rounded-2xl border border-gray-100 p-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            $ {current.price.toLocaleString()} /month · {current.delivery}
          </p>
        </div>
        <div className="flex gap-2">
          <button className="px-4 py-2 border border-gray-200 rounded-lg text-sm hover:bg-gray-50">Manage Billing</button>
          <button className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-red-600 hover:bg-red-50">Cancel Plan</button>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {REVENUE_MODEL.subscription.map((p) => (
          <div key={p.name} className={`bg-white p-6 rounded-2xl border ${user?.plan && planMap[user.plan] === p.name ? 'border-black ring-2 ring-black' : 'border-gray-100'}`}>
            ...
            <button disabled={planMap[user?.plan] === p.name}
              ...
              {planMap[user?.plan] === p.name ? 'Current Plan' : 'Upgrade'}
            </button>
          </div>
        ))}
      </div>
